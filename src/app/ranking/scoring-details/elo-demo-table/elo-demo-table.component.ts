...
export class EloDemoTableComponent {
  ...
  public EloPairs = input.required<EloDemoInput[]>();
  public Caption = input.required<string>();

  protected matches = computed(() => this.EloPairs()
    .map(pair => ({
  ...
  constructor() {
    effect(() => this.dataSource = new MatTableDataSource<EloScoreDemo>(this.matches()));
  }
}
